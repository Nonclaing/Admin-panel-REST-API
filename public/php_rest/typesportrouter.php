...
{
    // Гет запрос с путём
	$app->get('/rest/type_sports/get-list.json', function() use ($app, $json)
	{
...
function addTypeSport(Application $app, $json): void
{
	...
	$app->post('/rest/type_sports/add.json', function() use ($app, $json)
	{
        // Пытаемся добавить
		try
		{
...
function deleteTypeSport(Application $app, $json): void
{
	...
	$app->post('/rest/type_sports/delete.json', function() use ($app, $json)
	{
        // Пытаемся удалить
		try
		{
...
function updateTypeSport(Application $app, $json): void
{
	...
	$app->post('/rest/type_sports/update.json', function() use ($app, $json)
	{
        // Пробуем
		try
		{
            ...
            // Возвращаем запрос
			return $app->json([
				'item' => TypeSportsTable::getLastItem(),
				'error' => [
					'error' => false,
					'error_message' => ''
				]
