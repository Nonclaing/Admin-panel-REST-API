...
function addSportsman(Application $app, $json): void
{
	...
	$app->post('/rest/sportsmans/add.json', function() use ($app, $json)
	{
		...
		try
		{
            ...
            if (SportsmansTable::isNameExists($json['name']))
            {
                return $app->json([
                    'error' => [
                        'error' => true,
                        'error_message' => "Спортсмен ${json['name']} уже существует"
                    ]
                ])->setStatusCode(400);
            }
...
function updateSportsman(Application $app, $json): void
{
	...
	$app->post('/rest/sportsmans/update.json', function() use ($app, $json)
	{
		...
		try
		{
            ...
            // Проверка на существование имени
            if (SportsmansTable::isNameExists($json['name'], $json['id']))
            {
                return $app->json([
                    'error' => [
                        'error' => true,
			...
			if (!isset($data['ID']) && $data['ID'])
			{
				return $app->json([
					'error' => [
						'error' => true,
						'error_message' => "Вы пытаетесь изменить не существующего спортсмена"
					]
				])->setStatusCode(400);
			}
			...
			if (
                file_exists($_SERVER['DOCUMENT_ROOT'] . $data['FILE']) &&
                $json['photo'] != $data['FILE'])
			{
				unlink($_SERVER['DOCUMENT_ROOT'] . $data['FILE']);
			}
            ...
            $item = SportsmansTable::getDTO(
                [
                    'id' => 'sportsmans.id',
                    'name' => 'sportsmans.name',
                    'photo' => 'sportsmans.photo',
                    'biography' => 'sportsmans.biography',
                    'year' => 'sportsmans.year',
                    'type_sport' => [
                        'id' => 'type_sports.id',
